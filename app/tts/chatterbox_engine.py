...
class ChatterboxTTSEngine(BaseTTSEngine):
    def __init__(self, manager: ChatterboxManager | None = None) -> None:
        ...
        self._worker_config: tuple[str, str, str] | None = None
    ...
    def validate(self, voice_config: dict[str, Any]) -> None:
        if not self.manager.has_runtime():
            raise TTSEngineError(
                "Chatterbox is selected, but its Python runtime dependencies "
                "are not installed. Open Settings > TTS Engines and click Install."
            )
    ...
    def synthesize_to_wav(
        ...
        self._send_request(
            process,
            {
                ...
                "exaggeration": float(voice_config.get("exaggeration", 0.5)),
                "cfg_weight": float(voice_config.get("cfg_weight", 0.5)),
            },
        )
    ...
    def _start_worker(
        ...
        with self._lock:
            ...
            self._stdout_thread = threading.Thread(
                ...
                args=(process,),
                name="ChatterboxStdout",
                daemon=True,
            )
            self._stderr_thread = threading.Thread(
                target=self._read_stderr,
                args=(process,),
                name="ChatterboxStderr",
                daemon=True,
            )
    ...
    def _wait_for_ready(self, process: subprocess.Popen[str]) -> None:
        while True:
            ...
            if message_type == "ready":
                ...
                suffix = ", ".join(part for part in (model, device) if part)
                if suffix:
                    self.log_callback(f"Chatterbox worker ready ({suffix}).")
                else:
                    self.log_callback("Chatterbox worker ready.")
                return
    ...
    def _next_worker_message(
        ...
        while True:
            ...
            if message.get("type") == "stdout_closed":
                if process.poll() is not None:
                    raise TTSEngineError(
                        "Chatterbox worker stdout closed. "
                        + self._worker_error_details(process)
                    )
                continue
            if message.get("type") == "raw":
                self.log_callback(f"Chatterbox - {message.get('message', '')}")
                continue
            return message
    ...
    def _send_request(
        ...
        if process.stdin is None:
            raise TTSEngineError("Chatterbox worker stdin is not available.")
        try:
            process.stdin.write(json.dumps(request, ensure_ascii=False) + "\n")
            process.stdin.flush()
        except OSError as exc:
            raise TTSEngineError(
                "Could not send request to Chatterbox worker. "
                + self._worker_error_details(process)
            ) from exc
    ...
    def _log_worker_message(self, message: dict[str, Any]) -> None:
        ...
        if message_type == "timing":
            ...
            self.log_callback(f"Chatterbox timing - {label}: {elapsed:.3f} s")
    ...
    def _raise_for_worker_message(self, message: dict[str, Any]) -> None:
        ...
        if message_type == "shutdown":
            raise TTSEngineError("Chatterbox worker shut down unexpectedly.")
